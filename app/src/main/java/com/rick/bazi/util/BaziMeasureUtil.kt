...
//        println("Run [analyzeBaziAndSaveStat]")
        Log.i("[Rick]", "Aanalyze Bazi And Save Stat");

        baziModel.setBaziStr(WuXingUtil().createBaziStr(baziInfo))

        var ownerStr = WuXingUtil().getOwnerStr(baziInfo)
        ownerStr = ownerStr + stringResource(R.string.bazi_owner_brief)
        baziModel.setBaziOwnerStr(ownerStr)
        ...
        //deling check
        baziModel.setDeLingCheckStr(WuXingUtil().getDangLingStr(baziInfo, baziModel))
        baziModel.setDeDiCheckStr(WuXingUtil().getDeDiCheckStr(baziInfo, baziModel))

        //collect dedi info
        var isDeDi = WuXingUtil().isDeDi(baziInfo, baziModel)

        var deDiCheckStr = WuXingUtil().getDeDiCheckStr(baziInfo, baziModel)

        baziModel.setDeDiCheckStr(deDiCheckStr)
    ...
    fun getDayunCheckResult(tg : TianGan, dz : DiZhi, baziInfo: BaziInfo): String {
        ...
        @Composable
        fun getBaziGeJuSummary(baziInfo: BaziInfo, baziModel: BaziViewModel): String {
            var string = ""
            var gjString = ""
            var gjSummary = ""
...
//            Log.i("[Rick]", "GeJu tg=" + tg)
        ...
        fun getXiJiSummary(baziInfo: BaziInfo, baziModel: BaziViewModel): String {
            ...
            if (strenthLevel == BaziStrength.BAZI_STRONG_SUPER) {
                ...
                xiyongShenList = arrayListOf<WuXing>(WuXingUtil().getGuanshaWuXing(baziInfo.dayTiangan),WuXingUtil().getShishangWuXing(baziInfo.dayTiangan),WuXingUtil().getCaiWuXing(baziInfo.dayTiangan))
                jiShenList = arrayListOf<WuXing>(WuXingUtil().getYinWuXing(baziInfo.dayTiangan),WuXingUtil().getBiJieWuXing(baziInfo.dayTiangan))
